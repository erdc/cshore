...
c              FVCWLWT     = 0.5D0*VEGCDM(J,L)*VEGB(J,L)*VEGN(J,L)*
c     +                      2.0D0*(USIGT*SIGT)*(2.0D0*PI/WT(J))*0.5D0*
c     +                      (HRMS(J)*DSQRT(PI)*0.5D0)*
c     +                      2.0D0/PI*DSINH(WKP*MIN(H(J), VEGD(J,L)))/
c     +                      DSINH(WKP*H(J)) / WKP /GRAV
...
c     +   ((1.D0+VEGCDM(J,L)/VEGCD(J,L)*MIN(VEGH(J,L),H(J))*VEGFB(J,L))
c     +   *TBXSTA(J)+
c     +   STREAMSTRESSSTA-TWXSTA(ITIME))*DX)/H(J)
     ...
     +   STREAMSTRESSSTA-TWXSTA(ITIME))*DX)/H(J)
          ...
          OPEN (UNIT=5270, FILE='XLOCLWT.DAT', STATUS='UNKNOWN')
          WRITE(5270,*) XB(J)

          OPEN (UNIT=5273, FILE='FvegLWT.DAT', STATUS='UNKNOWN')
          WRITE(5273,*) -STREAMSTRESSSTA

          OPEN (UNIT=5278, FILE='FVCW2LWT.DAT', STATUS='UNKNOWN')
          WRITE(5278,*) FVCWLWT
        ENDIF 
...
c     +        DX*((1.D0+VEGCDM(JP1,L)/VEGCD(JP1,L)*MIN(VEGH(JP1,L),HITE)
c     +        *VEGFB(JP1,L))
c     +        *TBXSTA(JP1)+
c     +        (1.D0+VEGCDM(J,L)/VEGCD(J,L)*MIN(VEGH(J,L),H(J))
c     +        *VEGFB(J,L))*TBXSTA(J)
...
c     +                  2.D0*STREAMSTRESSSTA -2.D0*TWXSTA(ITIME)))/
c     +                  (HITE+H(J))
           OPEN (UNIT=5280, FILE='XLOC3LWT.DAT', STATUS='UNKNOWN')
           WRITE(5280,*) XB(JP1)
           OPEN (UNIT=5279, FILE='FVCW3LWT.DAT', STATUS='UNKNOWN')
           WRITE(5279,*) FVCWLWT
...
c     FINDHV2HTOMTABLE(URSELL,H2H,HV2H,FAOUT)   
c      CALL FINDHV2HTOMTABLE(URSELL,H2H,HV2H, FAOUT)
      CALL FINDHV2HTOMTABLE(URSELL,H2H,HV2H, HV2HTOMEME)
...
c/1.35D0 
            ...
            HS2H  = WHRMS*DSQRT(2.0D0) / D
            HV2H = 0.8D0
c TODO: separate submerged and emergent vegetation
c July 8.
            CALL FINDHV2HTOMEME(MIN(URSELL,1000.0D0),HS2H,HV2H,HV2HTOM)
