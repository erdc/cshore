...
leveeInfo1 = struct ('id', 'case1', 'd1', 9*0.3048, 'Tp',  7.7, 'Hrms', 2.0*0.3048, 'etabar', 0.0, 'SWL', 0.0) ; 
...
%%%%%
in.veg_n      = 450;       % vegitation density
...
plot (results.hydro.x, results.hydro.setup, 'b',  'linewidth', 2)
if in.iveg==0
    plot (results.hydro.x, results.hydro.Hrms, ':k',  'linewidth', 2)
elseif in.iveg>0
    plot (results.hydro.x, results.hydro.Hrms, ':r',  'linewidth', 2)
end
legend ('MWL (m)', 'H_{rms} (m)', 'location', 'northwest')
if in.iveg==0
    text (200, 3, ['no veg:'], 'fontsize', 15) ; 
    text (200, 2, ['R2%= ', num2str(results.hydro.runup_2_percent), ' (m)'], 'fontsize', 15) ; 
    text (200, 2.5, ['Qos = ', num2str(qosmodel), ' (cm2/s)'], 'fontsize', 15) ; 
elseif in.iveg>0
    text (230, 3, ['w/ veg:'], 'fontsize', 15) ; 
    text (230, 2, ['R2%= ', num2str(results.hydro.runup_2_percent), ' (m)'], 'fontsize', 15) ; 
    text (230, 2.5, ['Qos = ', num2str(qosmodel), ' (cm2/s)'], 'fontsize', 15) ; 
end    
title (['H_{rms, 0} = ', num2str(in.Hrms), ' (m)'])
