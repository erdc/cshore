...
% This code replace the levee with a slope so that there is no overtopping.

% load levee test information
leveeInfo1 = struct ('id', 'case1', 'd1', 9*0.3048, 'Tp',  7.7, 'Hrms', 3.6*0.3048, 'etabar', 0.0, 'SWL', 0.0) ; 
% leveeInfo1 = struct ('id', 'case1', 'd1', 9*0.3048, 'Tp',  7.7, 'Hrms', 0.3735*0.3048, 'etabar', 0.0, 'SWL', 0.0) ; 

% d1 is the offshore water depth 
figid = 11 ; 
% load Levee profile digitization
load LeveeProfile.txt 
...
for icase = 1
    ...
    Lx         = 350; %x_all (end); % can change to other values (e.g. 310, 350, etc) 
    Nx        = 4000 ;
    in.x       = linspace (0, Lx, Nx);
    in.dx     = in.x(2) - in.x(1) ;       % constant dx 
    
    in.zb      = interp1 (x_all, y_all, in.x, 'linear', 'extrap') ; 
    idtmp1     = find (in.x>=280) ; 
    idtmp2     = find (in.x>=260 & in.x<=280) ; 
    in.zb(idtmp1) = interp1(in.x(idtmp2), in.zb(idtmp2), in.x(idtmp1), 'linear', 'extrap') ; 
   
    in.veg_extent = [245/Lx, (245+40)/Lx] ;    
...
elseif in.iveg>0
    ...
    legend ('H_{rms} (m) no veg', 'H_{rms} (m) w/ veg', 'location', 'northwest')
end

idtmp = find (results.hydro.x>=260) ;
Hrmstoe = results.hydro.Hrms (idtmp(1)) ; 
if in.iveg==0
    text (180, 5, ['no veg: H_{rms, 0} = ', num2str(in.Hrms), ' (m)'], 'fontsize', 15) ; 
    text (180, 3.5, ['Qos = ', num2str(qosmodel), ' (cm2/s)'], 'fontsize', 15) ; 
    text (180, 2, ['R2%= ', num2str(results.hydro.runup_2_percent), ' (m)'], 'fontsize', 15) ; 
    text (180, 1.5, ['H_{rms, toe} = ', num2str(Hrmstoe), ' (m)'], 'fontsize', 15) ; 
elseif in.iveg>0
    text (230, 5, ['w/ veg: H_{rms, 0} = ', num2str(in.Hrms), ' (m)'], 'fontsize', 15) ; 
    text (230, 3.5, ['Qos = ', num2str(qosmodel), ' (cm2/s)'], 'fontsize', 15) ; 
    text (230, 2, ['R2%= ', num2str(results.hydro.runup_2_percent), ' (m)'], 'fontsize', 15) ; 
    text (230, 1.5, ['H_{rms, toe} = ', num2str(Hrmstoe), ' (m)'], 'fontsize', 15) ; 
end    
 ylim ([-4, 6])
...
elseif in.iveg==1
    plot (results.hydro.x, results.hydro.setup, 'r',  'linewidth', 2)
    legend ('MWL (m) no veg', 'MWL (m) w/ veg', 'location', 'northwest')
elseif in.iveg==3
    plot (results.hydro.x, results.hydro.setup, 'g',  'linewidth', 2)
    legend ('MWL (m) no veg', 'MWL (m) w/ veg (iveg=1)', 'MWL (m) w/ veg (iveg=3)', 'location', 'northwest')
end
xlim([100, Lx])

set (gcf, 'position', [1485          -1        1529         806])
