...
%     KC_jadhav       = Uwbase * (in.Tp/1.35) / 8e-3 ; 
    ...
    if length(in.x) == length(tot(:, 1))
        idtmp = find (in.zb> 0) ; 
        wavesetup (icase) = tot(idtmp(1), 2)  ;
        tot(idtmp(1), 1) 
    else
        fprintf ('The size of in.x and OSET doesn''t match! \n') ;
    end
    
end

figure(22); hold on; box on; grid on;
load Qosmodel_revetment.txt
if vegtest ==1 
    plot (Qosmodel_revetment, qosmodel, 'or', 'linewidth', 2, 'markersize', 10)
else 
    plot (Qosmodel_revetment, qosmodel, '+b', 'linewidth', 2, 'markersize', 10)
end
fplot (@(x) x, '-k', 'linewidth', 2, 'HandleVisibility', 'off')
axis equal
xlim ([0, 80])
ylim ([0, 80])
xlabel ('Q_{os} in Revetment Test Set (cm^2/s)')
ylabel ('Q_{os} in Vegetation Test Sets (cm^2/s)')
...
load Htoe4Overtoppingmodel_revetment.txt
if vegtest ==1 
    plot (Htoe4Overtoppingmodel_revetment, H_toe, 'or', 'linewidth', 2, 'markersize', 10)
else 
    plot (Htoe4Overtoppingmodel_revetment, H_toe, '+b', 'linewidth', 2, 'markersize', 10)
end
fplot (@(x) x, '-k', 'linewidth', 2, 'HandleVisibility', 'off')
fplot (@(x) 0.9*x, ':k', 'linewidth', 2, 'HandleVisibility', 'off')
fplot (@(x) 0.55*x, ':k', 'linewidth', 2, 'HandleVisibility', 'off')
text (0.14, 0.125, '10% reduction', 'fontsize', 15)
text (0.14, 0.065, '45% reduction', 'fontsize', 15)
axis equal
xlim ([0, 0.2])
ylim ([0, 0.2])
...
%%%
figure(44); hold on; box on; grid on;
load WaveSetup_revetment.txt
if vegtest ==1 
    plot (WaveSetup_revetment, wavesetup, 'or', 'linewidth', 2, 'markersize', 10)
else 
    plot (WaveSetup_revetment, wavesetup, '+b', 'linewidth', 2, 'markersize', 10)
end
fplot (@(x) x, '-k', 'linewidth', 2, 'HandleVisibility', 'off')
axis equal
xlim ([0, 0.06])
ylim ([0, 0.06])
xlabel ('$\overline{\eta}$ at $x=8.13$ m in Revetment Test Set (m)', 'interpreter', 'latex')
ylabel ('$\overline{\eta}$ at $x=8.13$ m in Vegetation Test Sets (m)', 'interpreter', 'latex')
legend ('Veg. Test Set 1', 'Veg. Test Set 2') 
set (gca, 'fontsize', 15)
set(gca,'linewidth',2)
...
if vegtest == 1 
    fid1 = fopen ('Qosmodel_vegtest1.txt', 'w') ; 
    fprintf (fid1, '%f ', qosmodel) ; 
    fclose (fid1) ; 

    fid2 = fopen ('Htoe4Overtoppingmodel_vegtest1.txt', 'w') ; 
