...
%     Lx       = 6.3+0.82+1.24;                  % length of domain
    ...
    idtmp    = find (in.zb_p >=Rc) ; 
    
    x_b       = [0,                        6.3,      6.3+0.82+1.24-0.105,             in.x(idtmp(1))] ;
    zb         = [-dt-0.1831,         -dt,         Rc,                                         in.zb_p(idtmp(1))] ; 
    in.zb     = in.zb_p; 
...
%%%%%%%%%%%%
fid1 = fopen ('Qosmodel_revetment.txt', 'w') ; 
fprintf (fid1, '%f ', qosmodel) ; 
fclose (fid1) ; 

fid2 = fopen ('Htoe4Overtoppingmodel_revetment.txt', 'w') ; 
fprintf (fid2, '%f ', H_toe) ; 
fclose (fid2) ; 

fid3 = fopen ('WaveSetup_revetment.txt', 'w') ; 
fprintf (fid3, '%f ', wavesetup) ; 
fclose (fid3) ; 
